...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    wheel_circumference = 2 * np.pi * r
    ticks_per_revolution = robot_spec["encoder_resolution"]
    meters_per_tick = wheel_circumference / ticks_per_revolution

    # Distancia recorrida por cada rueda
    d_left = left_encoder_ticks * meters_per_tick
    d_right = right_encoder_ticks * meters_per_tick

    # velocidad lineal y angular usando cintemtica diferencial 
    v = (d_left + d_right) / 2
    w = (d_right - d_left) / l
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                # Validar indices dentro del rango 
...
def generate_measurement_likelihood(segments, road_spec, grid_spec):
    ...
    for segment in segments:
        ...
        # verificar que los indices esten dentro del rango valido 
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        # Normalizar la creencia posterior para que sea una distribucion de probabilidad
        if np.sum(posterior_belief) != 0: # Evitar division entre cero
